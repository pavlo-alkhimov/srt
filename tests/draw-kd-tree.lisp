    ...
    (sdl:with-init ()
      ...
      (sdl:with-events ()
        ...
        (:idle ()
               ...
               (let* ((l (mapcar #'(lambda (x)(world->screen x)) (corners (patch-aabb patch))))
                      ...
                      (split-x (if (not (is-leaf (patch-kd-tree patch)))
                                   (world->screen (s (patch-kd-tree patch)))
                                   left)))
                 (sdl:with-color (color (sdl:color :r 150 :g 75 :b 0))
                   (iter (for i from 0 below (array-dimension (patch-vis patch) 0))
                   ...
                   (and (not (is-leaf (patch-kd-tree patch)))
                        (patch-kd-tree patch)
                        (l (patch-kd-tree patch))
                        (r (patch-kd-tree patch))
                        (not (is-leaf (l (patch-kd-tree patch))))
                        (not (is-leaf (r (patch-kd-tree patch))))
                        (let ((split-y-left  (world->screen (s (l (patch-kd-tree patch)))))
                              (split-y-right (world->screen (s (r (patch-kd-tree patch))))))
