  ...
  (if (< 0 recursion-steps-left)
      (with-dbg-header
          ...
          (let* ((triangles (if (not triangles-setp)
                                (iter (for x from 0 below (array-dimension (triangles-indexes-array patch) 0))
                                      (collect x))
                                triangles))
                 ...
                 l-triangles
                 r-triangles)
            (multiple-value-bind (split-position split-value split-array)
                (sah patch axis-index triangles)
